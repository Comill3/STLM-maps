...
def CleanLine(data_line, wavelet, eps, seuil_bas):
    ...
    while crit == 1 and step > 0:
        ...
        Conv1 = np.abs(
            np.real(np.fft.ifft(np.fft.fft(np.log(data_line)) * wavelet))
        )  # on convolue + abs
        ...
        if maxval > eps:
            ...
            minval, delta_arg = (
                np.amin(data_loc),
                np.argmin(data_loc),
            )  # on trouve le min local et son arg
            # si minval est en dessous du seuil bas, on garde delta_arg
            # sinon on prend celui du max
            if minval > seuil_bas:
                delta_arg = np.argmax(
                    data_line[argmax - 1 : argmax + 2]
                )  # on cherche localement la valeur max
            data_line = np.delete(
                data_line, argmax + delta_arg - 1
            )  # on enleve le point en question
