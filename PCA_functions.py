...
def perform_PCA(datapath, n):
    ...
    plt.savefig(os.path.join(savingpath, f'{n}vecteurs_propres.png'), dpi = 300, bbox_inches='tight')
    plt.show()
...
def project_on_PCA_basis(datapath, n, v_min, v_max):
    ...
    np.save(os.path.join(basispath, f'coeff_projection_from{start}to{stop}'), coeff_proj)