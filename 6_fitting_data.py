...
from fit_functions import gaussian, get_data_for_fit

DATA_PATH = "Demo_Data/STL_8"
PCA = True
Smooth = False
n = 7
Eph = 0.09  # Phonon replica energy in eV for GaN
BsA = 0.2  # Ration between QW emission and phonon replica for GaN

stl_data, stl_data_init, wavelength, energy, savepath, m, l = get_data_for_fit(
    PCA, DATA_PATH, n
)
...
for j in liste_spectra:
    ...
    def func1(x, A1, B1, E1, sigma1):
        ...
        return gaussian(x, np.abs(A1), E1, sigma1) + gaussian(
            x, np.abs(B1), E1 - Eph, sigma1
        )

    print(j)

    spectrum = stl_data[j]  # denoised data
    spectrum_init = stl_data_init[j]  # raw data
    ...
    # try & except: Prevents the program from crashing during execution if it encounters fitting issues
    try:
        # p0: guess parameters
        popt, pcov = curve_fit(
            func1,
        ...
        if A1 < 1:  # In case the spectrum is moslty noise with no light emitted
            coeff_fit_8G.append(
                [
                    8,
                    0,
    ...
    plt.plot(
        energy[i1:i2], gaussian(energy[i1:i2], np.abs(A1), E1, sigma1), color=couleur[1]
    )
    plt.plot(
        energy[i1:i2],
        gaussian(energy[i1:i2], np.abs(B1), E1 - Eph, sigma1),
        color=couleur[2],
    )
    plt.plot(energy[i1:i2], fit[i1:i2], "k", label="fit")

    plt.text(
        0.5, 1.07, s, ha="center", va="center", transform=ax.transAxes, fontsize=10
    )
    plt.text(
        0.14, 0.92, s2, ha="center", va="center", transform=ax.transAxes, fontsize=12
    )
...
np.savetxt(
    os.path.join(savepath, f"coeff_8Gaussian_fit_from{start}to{stop}.txt"),
    coeff_fit_8G,
    comments="# number of gaussian, and all gaussian param in order ampl, Energy, sigma",
    delimiter="\t",
)
