...
c = (
    ...
    / (1.29)
    * 2 ** (L / 2 - 1)
    * math.factorial(L / 2 - 1)
    / np.sqrt(L / 2 * math.factorial(L - 2))
)
...
def compute_hat_psi(pulsation_2d, theta, delta_n):
    ...
    hatpsi = c * np.exp(
        -np.power(np.abs(pulsation_2d) - xi0, 2)
        / (xi0**2 - (np.abs(pulsation_2d) - xi0) ** 2)
    )
    ...
    hatpsi = hatpsi * (
        (((np.angle(pulsation_2d) - theta) % (2 * np.pi)) <= (np.pi / 2.0)).astype(int)
        + (
            ((np.angle(pulsation_2d) - theta) % (2 * np.pi)) >= (3 * np.pi / 2.0)
        ).astype(int)
    )

    hatpsi = hatpsi * np.exp(
        -1j
        * delta_factor
        * (np.real(pulsation_2d) * delta_n_x + np.imag(pulsation_2d) * delta_n_y)
    )
...
def compute_hat_psi_j_theta_delta_n(j, l, delta_n):
    ...
    pulsation_2d = (
        ...
        * 2
        * np.pi
        * np.array([frequencies_x + 1j * frequency_y for frequency_y in frequencies_y])
    )
...
def compute_bank_of_wavelet(should_plot=False):
    ...
    for l in range(2 * L):
        ...
        for j in range(1, J + 1):
            ...
            for delta_n in range(max_delta_n + 1):
                psi_hat = (
                    compute_hat_psi_j_theta_delta_n(j, l, delta_n / 2)
                    / discrepency_factor
                )
                ...
                if should_plot and j == 3 and l <= 3:
                    plt.clf()
                    plt.imshow(
                        np.real(np.fft.ifftshift(np.fft.ifft2(psi_hat)))
                    )  # , vmin=-0.04, vmax=0.04)
                    plt.colorbar()
    ...
    result = {"filt_fftpsi": np.array(list_hat_psi), "filt_fftphi": hat_phi}
    filename = (
        "bump_steerable_wavelet_N"
        + str(N)
        + "_J"
        + str(J)
        + "_L"
