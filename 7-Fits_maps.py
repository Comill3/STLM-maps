...
dataname, basispath = get_gaussian_coeff(PCA, datapath, n, Gnumber, start, stop)
...
plt.imshow(L_A1, cmap=cmap1, aspect=1, extent=[image_size[1], 0, image_size[0], 0], interpolation='none', vmax=20) #, vmin=1.2, vmax=3.0)
...
savename = os.path.join(savingpath, Gnumber + 'G_Coeff_map_Gaussian1.png')
...
savename = os.path.join(savingpath, Gnumber + 'G_Hist_Gaussian1.png')
plt.savefig(savename, dpi = 300, bbox_inches='tight', format='png')
plt.show()
...
savename = os.path.join(savingpath, Gnumber + 'G_Coeff_map_Gaussian1-phonon-replica.png')
plt.savefig(savename, dpi = 300, bbox_inches='tight', format='png')
plt.show()
...
# savename = os.path.join(savingpath, 'Figure.png')
